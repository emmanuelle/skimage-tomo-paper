...
\address[id=aff2]{%
  \orgname{Life Sciences Computation Centre, University of Melbourne},
...
seconds~\cite{Maire2014}. New modalities such as single-bunch
imaging~\cite{Rack2014} provide a time resolution down to the nanosecond
...
due to artifacts~\cite{Marone2010}, or to an unforeseen evolution of
...
users to process their images. ImageJ~\cite{Abramoff2004, Schneider2012}
and its distribution Fiji~\cite{Schindelin2012} is a popular tool for 2-D
and 3-D images, thanks to its intuitive menus and graphical tools, and
the wealth of plugins contributed by a vivid
community~\cite{Schindelin2015}. For 3-D images, commercial tools such as
Avizo 3D software (TM) are appreciated for an intuitive graphical
...
limiting the programming task and the risk of bugs. Matlab (TM) and its image
processing toolbox are popular in the academic community of computer
vision and image processing. The Python language is widely used in the
scientific world and in synchrotron facilitiies. As a general-purpose
language, Python is used in synchrotrons for controlling device
servers~\cite{pytango}, reconstructing tomography volumes from
radiographies~\cite{Gursoy2014, Mirone2014}, and various data processing
packages for macromolecular cristallography~\cite{Adams2010} or
fluorescence analysis~\cite{pymca}.

\texttt{scikit-image}~\cite{Vanderwalt2014} is a general-purpose image
...
Python~\cite{Oliphant2007}. Like the rest of the ecosystem,
...
\texttt{NumPy} module~\cite{Vanderwalt2011}.
...
environment of Scientific Python~\cite{Oliphant2007, Perez2011}.
...
the IPython interpreter~\cite{Perez2007, Rossant2015}. IPython is an
...
The Jupyter notebook~\cite{Kluyver2016} is a web application which has
...
- advanced operations in a compact form, at minimal memory consumption

- slicing to reduce memory cost
...
\texttt{scikit-learn}~\cite{Pedregosa2011}, a Python module for machine
...
\begin{figure}
    \centerline{\includegraphics[width=0.99\columnwidth]{mayavi_porous}}
\caption{\csentence{Sample figure title.}
      A short description of the figure content
      should go here.}
\end{figure}
...
documented using the same standard~\cite{Pawlik2015} as other major
...
examples (url), a part of which is displayed in Fig.~\ref{fig:gallery}.
The principle of the gallery is to accelerate the learning curve of image
processing, by displaying graphical examples of image processing
...
Finally, tutorials on \texttt{scikit-image} are available in various places,
either as YouTube videos, or in the SciPy Lecture Notes~\cite{scipylecturenotes}, a comprehensive online book of Scientific Python tutorials.  
...
more than 170 developers that have contributed to the package. The
...
ensures that new features are proprosed to users on a regular basis.
