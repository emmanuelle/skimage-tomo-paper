...
\begin{abstract} % abstract
    ...
    simple programming interface for functions that perform various
    operations on images, grouped thematically according to their
    purpose, e.g. image restoration,
    segmentation, measurements, etc. \texttt{scikit-image} users benefit
    from a rich scientific Python ecosystem that contains many
    powerful libraries for tasks such as visualization or machine learning.
    \texttt{scikit-image} provides a gentle
...
limiting the complexity of the programming task and the risk of bugs. Matlab (TM) and its
...
Raw data formats can be opened using the \texttt{NumPy} functions \texttt{fromfile} or \texttt{memmap}, and \texttt{hdf5} files are accessed using modules such as \texttt{h5py}, \texttt{pytables}.
...
Most functions take both 2-D and 3-D images as arguments, so that
the same functions can be used to process tomography and microscopy
images. The rest raise an error
...
instructions and scripts interactively, the most popular in the scientific world is
IPython \citep{Perez2007, Rossant2015}. IPython is an
...
out of the IPython project. Jupyter notebooks provide an interactive development
...
im[im < 0.5] = 0
\end{lstlisting}
exploiting the possibility of indexing arrays with boolean arrays, also called
...
through mayavi's pipeline API.
\texttt{mayavi} offers a good trade-off between simplicity of use for
common operations, and accessibility to more sophisticated
capabilities such as responsive
visualizations.
...
of the local Hessian matrix is high (see Fig.~\ref{fig:tomo_gallery}).
...
algorithm aimed at images with strong contrast variations. Super-pixel
...
and spatial distance. Region-growing algorithms, such as the morphological
...
processing operations is of critical concern. \texttt{scikit-image} relies mostly
on calls to \texttt{NumPy} operations, of which most are performed in
optimized compiled code (C or Fortran). Performance-critical parts of \texttt{scikit-image}
...
Another situation that often arises is that, while prototyping a
workflow, scripts (containing the image processing pipeline) are run
several times to experiment with parameters. The \texttt{joblib}
...
The quality of software documentation is (perhaps especially)
important in software aimed at scientists.
\texttt{scikit-image} users have access to several kinds of
...
input and output variables and their data types, together with
explanations of what each function does and how to use it. Function
...
part of which is displayed in Fig.~\ref{fig:gallery},
showcases graphical examples of image processing
operations.  The examples are organized as an array of thumbnails with a
...
particular "big picture", foundational aspects of \texttt{scikit-image}, such as its
...
\texttt{scikit-image} website every month at \url{http://scikit-image.org/}. These
...
more than 170 developers having contributed to the package. The large
...
that more application-specific software will benefit by depending on
